            ...
            .message = [&rouletteWheel, &rouletteGame,&minesGame, &coinGame, &player,&currentPayout, &amountToFront, &game](auto *ws, std::string_view message, uWS::OpCode opCode) {
                ...
                try {
                    ...
                    if (message == "starting creds") {
                        ...
                        //Roulette case!
                        if (game == "roulette") {
                            if (startsWith(message, "total bet")) {
                                // Extract the total bet amount from the message
                                std::string totalBetStr = std::string(message.substr(10)); // Extract substring after "total bet" (10 characters)
                                int totalBet = std::stoi(totalBetStr);
                                cout <<"user had bet" << totalBet <<endl;

                                // Update player credits with the total bet amount
                                player->updateCredits(totalBet);
                            }

                            else if (message.substr(0,18) == "Button was clicked"){
                                ...
                                cout << "Bet String is : " << messageStr <<endl;
